...
- 现代化 UI：macOS 26 液态玻璃（Liquid Glass）风格，向下版本平滑降级
...
macOS 26 液态玻璃（Liquid Glass），无渐变光晕动效（用户选定方案 A）：
- 主容器使用 `NSVisualEffectView`（`.hudWindow` 或自定义 material）营造全窗口半透明毛玻璃背景，
  控件浮在模糊背景上，带微妙折射光泽
- 按钮和卡片使用高斯模糊 + 轻微边框高光，不做步骤切换时的彩色渐变光晕过渡动效
