...
刚刚完成 **OpenSSL 3.6.2 universal dylib 自包含集成** 并推送至 `private/main`
（commit `e4aa9b8`，2026-07-08 22:54）。EraseA12 二进制已不再依赖
`/usr/local/opt/openssl@3/...`，`@rpath/libssl.3.dylib` / `@rpath/libcrypto.3.dylib` 嵌入
在 `EraseA12.app/Contents/Frameworks/` 内，任意没装 Homebrew OpenSSL 的 Mac 都能直接启动。
DMG 19M、SHA-256 `3d09e4f7...`、39/39 测试通过、严格签名通过。当前空闲，等用户决定下一步
（vendor 体积是否压缩、真机擦除、Developer ID 公证、CI 等）。
...
- vendor 库体积偏大（~34 MB），其中 `.a` 静态库（~21 MB）是 build 时副产品，待用户决定是否
  加 `.gitignore` 排除仅保留 dylib。
...
- vendor `libssl.a` / `libcrypto.a`（~21 MB）目前会进仓库，跟 `libirecovery.a` 一致；
  DMG 实际只需要两个 dylib。如需压缩可加 `.gitignore` 排除 `Vendor/openssl/lib/*.a`。
...
1. 用户决策 vendor 体积：是否加 `.gitignore` 排除 `Vendor/openssl/lib/*.a`（仅保留 dylib 和头文件）。
2. 在干净环境安装 xcodegen，从 `project.yml` 重建工程并复跑 39 项测试及 Release 构建。
