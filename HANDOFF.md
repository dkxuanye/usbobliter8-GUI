...
刚刚完成 **vendor 体积瘦身**：`.gitignore` 排除 `EraseA12/Vendor/openssl/lib/*.a`
（`libssl.a` + `libcrypto.a` 共 ~21 MB），并从 git 索引 `git rm --cached` 移除。工作树
仍保留 .a（供本地 build 备用），新 clone 后需要静态链接时可重跑 `Scripts/build-openssl.sh`
重编（~8 分钟）。验证：`SKIP_DMG_FINDER_LAYOUT=1 ./Scripts/package-dmg.sh` 跑通，DMG 19M、
SHA-256 `e6f5e85d...`、严格签名通过、`@rpath/libssl/libcrypto.3.dylib` 自包含。

> ⚠️ `make release` 强依赖 `xcodegen`，本机未装时需直接调 `./Scripts/package-dmg.sh`（脚本
> 自身已兼容无 xcodegen + 本地已有 `.xcodeproj` 的 fallback）。
...
- 2026-07-08 21:50 修 `Scripts/package-dmg.sh` bug：`SKIP_DMG_FINDER_LAYOUT=1` 路径之前只 echo 跳过
  没有真生成 DMG，现在直接 `hdiutil create -format UDZO` 从 staging 打包。
- `EraseA12/Makefile` 新增 `release` 和 `dmg` 一键目标；`AGENTS.md` 改"编译 = clean Release + DMG"为标准流程。
- 2026-07-08 22:08 集成 OpenSSL dylib（开箱即用）：
  - 新增 `Scripts/build-openssl.sh`：编译 OpenSSL 3.6.2 universal dylib（x86_64 + arm64）到 `Vendor/openssl/lib/`
  - 新增 `Scripts/bundle-openssl.sh`：把 dylib 嵌入 `.app/Contents/Frameworks/`，改 install name + RPATH + 重新签名
  - `package-dmg.sh` 插入 `[2.5/6] 嵌入 OpenSSL dylib` 步骤（必须在 build+sign 之后，staging 之前）
  - EraseA12 二进制动态依赖从 `/usr/local/opt/openssl@3/...` 改为 `@rpath/libssl.3.dylib` / `@rpath/libcrypto.3.dylib`
  - 严格签名通过，otool -l 验证 LC_RPATH `@executable_path/../Frameworks` 正确
  - DMG 19M（之前 14M + 5M OpenSSL），SHA-256 `3d09e4f797f168453198c5d02f1821247addbdfd4ffb6f0ad3f9f295b2efe9cd`
- 最新 DMG 产物：`/Users/dkxuanye/Desktop/usbobliter8/EraseA12-1.0.0.dmg`（19M，universal x86_64+arm64，自包含 OpenSSL）。
...
- 2026-07-08 22:54 推送 OpenSSL 集成至 `private/main`（commit `e4aa9b8`，154 files, +46526/-17）：
  包含 `Scripts/build-openssl.sh`、`Scripts/bundle-openssl.sh`、`Vendor/openssl/` 下两个
  universal dylib + 两个 universal `.a` 备用 + 134 个头文件（vendor 整体 ~34 MB）。
  同步推送：DMG 流程固化（`5b5036c`）、联网更新检查设计与实现（`77b2d99`、`5b5036c`）。
...
- 暂未在干净克隆环境下用 `xcodegen generate` 重建工程并复跑 39 项测试。
...
- vendor `libssl.a` / `libcrypto.a`（~21 MB）已加 `.gitignore` 排除，git 索引已 `rm --cached`
  移除。工作树仍保留 .a 供本地 build 备用。
...
1. 在干净环境安装 xcodegen，从 `project.yml` 重建工程并复跑 39 项测试及 Release 构建。
2. 顺手修 `EraseA12/Makefile` 的 `release`/`dmg` 目标：xcodegen 缺失时 fallback 到已存在的
   `EraseA12.xcodeproj`（仿照 `package-dmg.sh` 的兼容逻辑），免得每次都得绕过 Makefile。
...
6. 配置 Developer ID 签名与 Apple 公证，DMG 才能跨机器免 Gatekeeper 警告。
7. 把 update.json 部署到 `https://www.dkxuanye.fit/EraseA12/update.json`，首次发版用 `1.0.0`。
