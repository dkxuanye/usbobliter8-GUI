...
已完成标准 macOS 应用菜单和“关于 EraseA12”窗口，展示原作者、MIT 版权、原生 GUI 开发者
署名及网站链接。功能已通过测试和 Release 构建，当前处于分支收尾阶段。
...
- 新增界面中文、布局和关于窗口回归测试；当前完整测试基线为 30 项。
...
- 2026-07-08 15:31 验证：30/30 测试、clean Release、严格签名、universal 架构、资源和实际启动通过。
...
2. 在干净环境安装 xcodegen，从 `project.yml` 重建工程并复跑 30 项测试及 Release 构建。
...
4. `EraseA12/EraseA12/App/AppDelegate.swift` - 应用菜单、关于窗口和应用生命周期。
5. `EraseA12/EraseA12/App/Localization.swift` - 固定简体中文的实现入口。
6. `EraseA12/EraseA12Tests/StepIndicatorViewTests.swift` - 中文界面、布局和关于窗口回归测试。
